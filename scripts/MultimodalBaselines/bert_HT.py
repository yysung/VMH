...
def parser(sent):
    sentence = sent['alternatives'][0]['transcript']
    start = sent['word_alternatives'][0]['start_time']
    end = sent['word_alternatives'][-1]['end_time']
    return {'start': start, 'end': end, 'text': sentence}

def parse_sentence(transcripts):
    sentences = {'start': [], 'end': [], 'text': []}
    current_start_time = transcripts[0]['start']
    current_text = ''
    
    for item in transcripts:
        current_text += ' ' + item['punctuated_word']
        if item['punctuated_word'].endswith('.') or \
           item['punctuated_word'].endswith('!') or \
           item['punctuated_word'].endswith('?') :
               current_end_time = item['end']
               
               sentences['start'].append(current_start_time)
               sentences['end'].append(current_end_time)
               sentences['text'].append(current_text.strip())
               
               current_start_time = item['end']
               current_text = ''
...
#dat_ = pd.read_csv('D:/videoclip/fairseq/examples/MMPT/data/vmh/2247_features_UPDATE.csv',index_col=0)
...
for random_seed in np.arange(5):
    ...
    train_dat = [
        dict(zip(['text', 'label'],
            (dat_.iloc[i]['headline'] + ' [SEP] ' + dat_.iloc[i]['transcripts'] if dat_.iloc[i]['transcripts'] != '' else dat_.iloc[i]['headline'],
    ...
    avg_pre_stack.append(average_precision_score(true_labels, pred_labels))

...
class TranscriptProcessor(Processor):
    ...
    def __call__(self, text_id):
        txt = text_id + ' [SEP] ' + self.df[text_id == self.df['headline']]['transcripts'].values[0]
        caption = self.tokenizer(txt, add_special_tokens=False)
        return caption["input_ids"]

class VHTsentHsentTvenue_Processor(Processor):
    """
    A generic Text processor: rename this as `withTokenizer`.
    tokenize a string of text on-the-fly.
    Warning: mostly used for end tasks.
    ...
    def __call__(self, text_id):
        txt = text_id + ' [SEP] ' + self.df[text_id == self.df['headline']]['transcripts'].values[0] + \
            ' [SEP] ' + self.df[text_id == self.df['headline']]['headline_sentiment'].values[0] + \
            ' [SEP] ' + self.df[text_id == self.df['headline']]['transcript_sentiment'].values[0] + \
            ' [SEP] ' + self.df[text_id == self.df['headline']]['venues'].values[0]
        #print(txt)
        caption = self.tokenizer(txt, add_special_tokens=False)
        return caption["input_ids"]

class VHTsentH_Processor(Processor):
    """
    A generic Text processor: rename this as `withTokenizer`.
    tokenize a string of text on-the-fly.
    Warning: mostly used for end tasks.
...
class VHTsentT_Processor(Processor):
    """
    A generic Text processor: rename this as `withTokenizer`.
    tokenize a string of text on-the-fly.
    Warning: mostly used for end tasks.
...
class VHTvenue_Processor(Processor):
    """
    A generic Text processor: rename this as `withTokenizer`.
    tokenize a string of text on-the-fly.
    Warning: mostly used for end tasks.
    ...
    def __call__(self, text_id):
        txt = text_id + ' [SEP] ' + self.df[text_id == self.df['headline']]['transcripts'].values[0] + \
            ' [SEP] ' + self.df[text_id == self.df['headline']]['venue'].values[0]
        #print(txt)
        caption = self.tokenizer(txt, add_special_tokens=False)
        return caption["input_ids"]
    

class TranscriptRationaleProcessor(Processor):
        """
        A generic Text processor: rename this as `withTokenizer`.
        tokenize a string of text on-the-fly.
        Warning: mostly used for end tasks.
        ...
        def __call__(self, text_id):
            txt = text_id + ' [SEP] ' + self.df[text_id == self.df['headline']]['transcripts'].values[0] + \
                ' [SEP] ' + self.df[text_id == self.df['headline']]['rationales'].values[0]
            #print(txt)
            caption = self.tokenizer(txt, add_special_tokens=False)
            return caption["input_ids"]
        
        
class TranscriptRandomRationaleProcessor(Processor):
        """
        A generic Text processor: rename this as `withTokenizer`.
        tokenize a string of text on-the-fly.
        Warning: mostly used for end tasks.

...
def get_dataloader(config, df, train=True):
    ...
    text_processor_cls = getattr(processors, 'TranscriptRationaleProcessor')
    ...
    text_processor = text_processor_cls(config.dataset ,df)
...
for random_seed in np.arange(5):
    ...
    print(f'Best val epoch {best_val_epoch:03f} val f1 {best_val_f1:.3f}')

...
for random_seed in np.arange(5):
    ...
    train_dat = [
        dict(zip(['text', 'label'],
            (dat_.iloc[i]['headline'], dat_.iloc[i]['majority_answer']))) for i in tr_idx]
    
    val_dat = [
        dict(zip(['text', 'label'],
            (dat_.iloc[i]['headline'], dat_.iloc[i]['majority_answer']))) for i in val_idx]
    
    test_dat = [
        dict(zip(['text', 'label'],
            (dat_.iloc[i]['headline'], dat_.iloc[i]['majority_answer']))) for i in te_idx]
    
    print('train:',len(train_dat),'val:', len(val_dat), 'test:',len(test_dat))
